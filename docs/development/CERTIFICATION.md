...
| Dependency sanity    | lockfile committed; `bun install` reproducible; no known-vulnerable direct deps flagged by audit tooling at release time | ✔                                     | bun pm / review                       |
...
3. `bun run certify` → must exit 0.
4. Review `verification/report.json` (gate list, timestamps, commit).
